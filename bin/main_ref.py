...
AUTOMOD_CHANNEL = os.getenv("AUTOMOD")
adminprefix = "admin_bot: "
Chat_key = "j"

keyinput_lock = asyncio.Lock()
hardrestart_in_progress = False
...
game_ini_path = os.path.join(os.getenv("LOCALAPPDATA"),"BrickRigs","SavedRemastered","Config","WindowsNoEditor","Game.ini")
...
async def send_banned_vehicle_message(player_name, vehicle):
    channel = bot.get_channel(AUTOMOD_CHANNEL)
    if channel:
        await channel.send(
...
async def checkmatch():
    if hardrestart_in_progress:
        return

    try:
        location = pyautogui.locateOnScreen("Orangewin.png", confidence=0.6)
        if location:
            screen_height = pyautogui.size().height
            image_bottom = location.top + location.height
            if image_bottom >= screen_height * 0.7:
                async with keyinput_lock:
                    if hardrestart_in_progress:
                        return
...
async def run_hardrestart():
    global hardrestart_in_progress
    async with keyinput_lock:
        hardrestart_in_progress = True
        try:
            await keyinputs.sendmessage("AutoMod", "Server rebooting in 5 seconds.")
            await asyncio.sleep(5)
            await keyinputs.hardrestart()
...
@bot.command(name="hardrestart")
@commands.has_role("Bot Admin")
async def hardrestart(ctx):
    await ctx.send("Restarting server...")
    await run_hardrestart()
